...
- 公开接口使用单并发队列、最小请求间隔和短期内存缓存，降低 403/429 限流概率
...
3. 选择 10 分钟、1 小时、24 小时或 7 天；插件会把完整的 `LDGS1` 分享码复制到剪贴板。
4. 把分享码发给对方。对方在 VS Code 或 PyCharm 插件中选择“打开临时分享码”，粘贴后即可打开同一个公开主题。
5. 也可执行 `LINUX DO: 临时分享码使用说明`，随时查看插件内教程。

分享码格式是 `LDGS1.<Base64URL 载荷>.<SHA-256 校验和>`。载荷包含版本、主题编号、slug、标题、生成时间 `iat` 和过期时间 `exp`。Base64URL 只是把文本转换为适合复制的字符，不是加密，因此接收方也没有“解密”步骤。末尾 16 个十六进制字符是 SHA-256 摘要的前 8 字节，用于发现误粘贴、损坏或修改，不是数字签名或访问控制。

分享码没有盐或秘密密钥。把固定盐或密钥放进 VS Code/PyCharm 这两个公开插件中不能保密，因为任何人都可以从安装包中取出它。过期时间会被两个插件校验，过期后拒绝导入；但公开主题 URL 仍可从分享码中还原，已经打开或另行保存的 URL 无法撤回。分享码不会包含 Cookie、User-Agent、搜索条件、阅读历史或滚动位置。
...
为避免快速切换页面或连续滚动触发站点限制，所有接口请求会合并相同 URL、按顺序发送，并在两次真实请求之间保留最小间隔。最新/热门列表缓存约 90 秒，搜索缓存约 60 秒，分类和主题缓存更久；缓存只存在于当前扩展进程内。手动刷新会请求新数据，但仍遵守请求间隔。遇到 403 或 429 后插件会进入冷却期，冷却期间不会继续联网；若已有不超过 30 分钟的缓存，则先显示缓存内容。持续出现 403 时再更新 Cookie 与 User-Agent，不要连续点击刷新。
