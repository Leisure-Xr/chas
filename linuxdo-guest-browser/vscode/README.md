...
- [仓库最新 VSIX（0.19.0）](https://github.com/Leisure-Xr/chas/raw/main/linuxdo-guest-browser/dist/linuxdo-guest-browser-vscode-0.19.0.vsix)
- SHA-256 以[项目下载表](https://github.com/Leisure-Xr/chas/tree/main/linuxdo-guest-browser#下载)为准。

仓库和 `dist/` 目录只保留 `0.19.0` 最新 VSIX，旧版本安装包已清理。
...
- 使用多个本地收藏目录整理公开主题，支持新建、重命名、删除目录以及打开、复制和移除收藏
- 在 VS Code 内阅读帖子
...
工具栏星标按钮打开收藏夹。主题页的“收藏”按钮可以选择现有目录或新建目录；收藏夹支持搜索、重命名和删除目录，以及打开、复制 URL、从当前目录移除主题。同一主题可以放进多个目录，每个目录内自动去重。收藏只保存目录名、公开主题标题、规范 URL 和收藏时间。
