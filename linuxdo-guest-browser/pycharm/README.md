...
- [仓库最新 ZIP（0.14.4）](https://github.com/Leisure-Xr/chas/raw/main/linuxdo-guest-browser/dist/linuxdo-guest-browser-pycharm-0.14.4.zip)
- SHA-256（当前构建包）：`49244e0b70d3a91e0517dda340ce2f259dc53cb43a51fe612bb8a44335f6500f`

仓库和 `dist/` 目录只保留 `0.14.4` 最新 ZIP，旧版本安装包已清理。

最低支持 PyCharm 2022.3（构建号 `223`），上限为 `263.*`（2026.3）。需要 JetBrains Runtime 自带 JCEF；工具窗口会在缺少 JCEF 时给出提示。`0.14.4` 在刷新时显式跟踪主文档加载，并在 JCEF 不再发送主框架 `onLoadEnd` 时由全局加载停止事件完成样式注入；完成路径是幂等的，不会与正常主框架回调重复执行。`0.14.3` 增加的内容可见性回退继续保留；页面加载、登录重定向或网络失败也会显示可重试错误页。
...
`0.13.0` 修复 Discourse SPA 跳转不触发完整页面加载时的历史漏记；JCEF 主框架地址变化后会立即新增记录，稍后到达的标题继续更新同一条记录。`...` 菜单新增“收藏当前主题”和“收藏夹”，支持多个目录、新建/重命名/删除目录、搜索收藏，以及打开、复制或移除公开主题。
...
工具窗口默认使用自适应的单色隐私阅读布局，不添加伪代码、文件标签或行号装饰。主题使用简洁标题，作者只显示单色纯用户名。帖子正文以外的图片、SVG、视频、头像、徽章、表情、用户卡、用户头衔和状态装饰全部隐藏；标题、标签、链接、状态、边框和背景跟随 IDE 主题。帖子正文中的截图、附件、Onebox 和技术图片保持原色。

固定顶栏在任何宽度都只保留返回、前进、刷新和 `...`。最新、热门、分类及搜索保留在下一行，窄窗口会自动分行。历史、收藏夹、休息提醒、小游戏、分享、导入分享、分享教程、隐私/原始布局切换和重置会话只出现在 `...` 菜单，不显示重复工具栏按钮。游客会话初始化期间点击的最后一次导航会保留；点击当前入口会刷新，加载完成后按实际 URL 高亮。原始网页布局仍可从 `...` 菜单恢复。
...
收藏夹按用户创建的目录保存公开主题。同一主题可以放入多个目录，目录内自动去重；删除目录只删除该目录中的收藏，不影响浏览历史或其他目录。收藏数据只包含目录名、公开主题标题、规范 URL 和收藏时间。
...
- 插件没有账号输入框，也不包含统计代码。通过 PyCharm 属性服务保存隐私布局和休息提醒设置、每款游戏一个整数最高分、最多 60 条公开历史以及本地收藏目录。历史和收藏不包含 Cookie、正文或登录状态。
...
```text
build/distributions/linuxdo-guest-browser-pycharm-0.14.4.zip
```
